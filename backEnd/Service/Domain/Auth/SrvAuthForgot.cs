...
{
    public class SrvAuthForgot : SrvBaseService
    {
        public IUserRepo userRepo = new UserRepo();
        public IUserPassRenewalRepo userPassRenewallRepo = new UserPassRenewalRepo();

        public bool ForgotPasswordMobile(string conn, string mobile, int expMinutes)
        {
            ...
            if (expMinutes < 1)
                return ReportError("Configuration information failed");
